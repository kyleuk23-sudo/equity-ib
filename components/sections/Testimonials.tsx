...
export function Testimonials() {
  ...
  const activeColor = TIER_COLORS[testimonials[active].tier];
  ...
  return (
    <section className="py-24 relative overflow-hidden">
      <div
        ...
        style={{ background: "radial-gradient(ellipse 50% 50% at 20% 50%, rgba(200,149,42,0.08) 0%, transparent 70%)" }}
      />
        ...
        <div className="max-w-3xl mx-auto relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="glass-strong rounded-2xl p-8 sm:p-10 relative overflow-hidden shadow-glass"
            >
              <div
                className="absolute inset-0 opacity-30 pointer-events-none"
                style={{
                  background: `radial-gradient(circle at 100% 0%, ${activeColor}15 0%, transparent 60%)`,
                }}
              />
              ...
              <p className="text-lg text-stone-200 leading-relaxed mb-8 relative">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-semibold text-stone-100">{testimonials[active].name}</div>
                    <div className="text-sm text-stone-400">{testimonials[active].role}</div>
                    <div className="text-xs text-stone-400 mt-0.5">{testimonials[active].country}</div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-stone-400">{testimonials[active].lots}</div>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
              ...
              {testimonials.map((_, i) => (
                ...
                >
                  <span
                    ...
                    style={{
                      background: i === active ? "#C8952A" : "rgba(255,255,255,0.15)",
          ...
          {testimonials.map((t, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <div>
                  <div className="text-xs font-medium text-stone-100 leading-tight">{t.name.split(" ")[0]}</div>
                  <div className="text-xs" style={{ color: TIER_COLORS[t.tier] }}>{t.tier}</div>
                </div>
              </div>
              <div className="text-xs text-stone-400 truncate">{t.lots}</div>
            </button>
          ))}
        </div>
      </div>
