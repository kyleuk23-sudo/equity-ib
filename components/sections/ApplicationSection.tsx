...
import { CheckCircle2, ArrowRight, DollarSign, Users, Clock, Shield, Globe, BarChart3 } from "lucide-react";
...
const EMPTY: FormData = { name: "", email: "", phone: "", telegram: "", country: "", broker: "", lots: "", message: "" };
...
export function ApplicationSection() {
  ...
  const inputCls = "w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-primary/50 focus:bg-white/[0.06] transition-all";
            ...
            <div className="glass rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <div className="flex -space-x-2">
                  {["JW","PS","MD","AA"].map((ini) => (
                    <div key={ini} className="w-7 h-7 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-[9px] font-bold text-white border border-background">
                      {ini}
                    </div>
                  ))}
                </div>
                  ...
                  <motion.form key="form" onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input value={form.phone} onChange={set("phone")} placeholder="+1 234 567 8900" className={inputCls} />
                      </div>
                      <div>
                        <label className="text-xs text-slate-400 mb-1.5 block">Telegram Handle</label>
                        <input value={form.telegram} onChange={set("telegram")} placeholder="@yourusername" className={inputCls} />
                      </div>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input required value={form.country} onChange={set("country")} placeholder="United Kingdom" className={inputCls} />
                      </div>
                      <div>
                        <label className="text-xs text-slate-400 mb-1.5 block">Current Broker</label>
                        <input value={form.broker} onChange={set("broker")} placeholder="e.g. IC Markets" className={inputCls} />
                      </div>
                    </div>

                    <div>
                      <label className="text-xs text-slate-400 mb-1.5 block">Estimated Monthly Volume *</label>
                      <select required value={form.lots} onChange={set("lots")} className={`${inputCls} bg-[#0a0a14]`}>
                    ...
                    <div>
                      <label className="text-xs text-slate-400 mb-1.5 block">Tell Us About Your Client Base</label>
                      <textarea
